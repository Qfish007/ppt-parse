...
/**
 * 获取百度语音 URL
 * 如果是 http 协议，使用本地代理 `/tts/baidu?text=xxx&spd=xxx`
 * 否则直接使用百度翻译 TTS 接口
 * @param {string} text - 要发音的文本
 * @param {number} [rate] - 播放速率
 * @returns {string} 语音 URL
 */
export function baiduVoiceUrl(text, rate) {
  const spd = baiduSpeed(rate);
  if (typeof window !== 'undefined' && window.location.protocol.startsWith('http')) {
    return `/tts/baidu?text=${encodeURIComponent(text)}&spd=${spd}`;
  }
 ...
 * @returns {Promise}
 */
export function playBaiduText(text, runId, playAudioUrl, normalizeSpeechText, rate, fallbackProvider = 'browser') {
  const value = normalizeSpeechText(text);
  if (!value) return Promise.resolve();
  return playAudioUrl(value, runId, baiduVoiceUrl(value, rate), fallbackProvider);
}
