<template>
    <div class="project-detail">
        ...
        <template v-else>
            ...
            <div v-if="isTranslationProject(project.type)" class="upload-section translation-section">
                ...
                <div class="translation-input-card">
                    <label class="field-label">{{ project.type === 'translate-en' ? '请输入英文（单词或段落）' : '请输入中文（单词或段落）'
                        }}</label>
                    <el-input v-model="translationInput" type="textarea" :autosize="{ minRows: 5, maxRows: 10 }"
                        :placeholder="project.type === 'translate-en'
                            ? '输入英文内容，点击录入后自动翻译为中文。每次录入相当于一页。'
                            : '输入中文内容，点击录入后自动翻译为英文。每次录入相当于一页。'" @keyup.ctrl.enter="submitTranslationEntry" />
                    <div class="translation-input-actions">
                        <el-button type="primary" size="large" :icon="Upload" :loading="isTranslating"
                ...
                <div v-if="pages.length > 0" class="page-nav translation-page-nav">
                    <div v-for="(page, idx) in pages" :key="page.page || idx" class="page-nav-item"
                        :class="{ active: bookStore.currentIndex === idx }" @click="selectPage(idx)">
                        <span>第 {{ idx + 1 }} 页</span>
            ...
            <div v-else-if="project.type === 'image'" class="upload-section image-project-section">
                ...
                <div v-if="pages.length > 0" class="page-nav image-page-nav">
                    <div v-for="(page, idx) in pages" :key="page.image || idx" class="page-nav-item"
                        :class="{ active: bookStore.currentIndex === idx }" @click="selectPage(idx)">
                        <span>第 {{ idx + 1 }} 页</span>
            ...
            <div v-else-if="pages.length > 0" class="page-nav">
                <div v-for="(page, idx) in pages" :key="idx" class="page-nav-item"
                    :class="{ active: bookStore.currentIndex === idx }" @click="selectPage(idx)">
                    第 {{ idx + 1 }} 页
            ...
            <div v-else class="upload-section">
                ...
                <div v-if="project.type !== 'image' && uploadedFiles.length > 0" class="file-list-area">
                    ...
                    <div class="parse-action">
                        <el-button type="success" size="large" :icon="Aim" :loading="isParsing" @click="startParse">
                            开始识别
                        </el-button>
                    </div>
                </div>
            ...
            <div v-if="isParsing" class="parse-progress-bar">
                <div class="parse-progress-header">
                    <el-icon class="is-loading" color="#126b62">
                        <Loading />
                    </el-icon>
        ...
        <!-- 隐藏的文件输入 -->
        <input ref="fileInputRef" type="file" :accept="acceptTypes" :multiple="project?.type === 'image'"
...
.page-nav-item.active .parsed-badge {
    color: #a0ebc0;
}
