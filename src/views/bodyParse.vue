  ...
  <section class="panel-right" :style="{ flex }">
    <div class="reader-header">
      ...
      <div class="reader-actions">
        ...
        <el-button size="small" type="primary" :disabled="!currentPage?.lines?.length" @click="$emit('read-current-page')">
          ...
          <template v-if="isTransProject">
            <template v-if="orderedLine(group)[1] === 'en'">
              <div class="study-lang-row">
                ...
                <p class="english paragraph-text" :class="{ 'lesson-title': index === 0 && isHeading(orderedLine(group)[0]) }">
                  <template v-for="(part, ti) in tokenize(orderedLine(group)[0])" :key="'f-'+ti">
                    ...
                    <button
                      ...
                      @click.stop="$emit('word-click', $event, part)"
                    >{{ part }}</button>
                  </template>
                </p>
              </div>
            ...
            <template v-else>
              <div class="study-lang-row">
                ...
                <div
                  ...
                  @input="$emit('first-lang-edit', $event, group, 'zh')"
                ></div>
              </div>
            </template>
          ...
          <template v-else>
            ...
            <div class="study-lang-row">
              ...
              <div
                ...
                data-placeholder="在这里补中文对照"
                :innerHTML="group.zh"
                @input="$emit('chinese-edit', $event, group)"
              ></div>
            </div>
          </template>
        </section>
